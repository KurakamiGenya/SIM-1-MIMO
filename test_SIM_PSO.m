...
w = 0.7; %% Inertia weight (CONSTANT - Standard PSO characteristic)
...
for ii = 1:Max_L
    ...
    for jj = 1:MonteCarlo
        ...
        for iter = 1:max_iterations
            ...
            % Display progress
            if mod(iter, 20) == 0
                fprintf('L=%d, MC=%d, Iter=%d, Best Capacity=%.4f bits/s/Hz\n', L, jj, iter, gBest_fitness);
            end
        end
...
ylabel('NMSE');
title('NMSE vs Number of TX-SIM Layers (PSO)');
grid on;
...
title('Spectral Efficiency vs Number of TX-SIM Layers (PSO)');
grid on;

%% Save results
NMSE_K_10_PSO = NMSE_average;
save('NMSE_K_10_PSO.mat', 'NMSE_K_10_PSO');

Capacity_K_10_PSO = Capacity_average;
save('Capacity_K_10_PSO.mat', 'Capacity_K_10_PSO');
